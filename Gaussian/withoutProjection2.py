...
def sampleMultinormal(mean, var):
    d = len(mean)
    mn = np.zeros(d, float)
    for i in range(d):
        mn[i] = mean[i, 0]
    
    return np.matrix(np.random.multivariate_normal(mn, var)).getT()
...
class multivariateGaussian:
    ...
    def getSample(self):
        ...
        while(not bincl):
            ...
            xf = self.std * xs
            xf = xf + self.mean
            #xf = sampleMultinormal(self.mean, self.var)
...
class truncatedLearning:
    ...
    def basicSGD(self, iterNum=5000, displayStep = 10):
        ...
        mn = np.zeros((self.d, 1), float)
        var = diag(np.ones((self.d, 1), float)) + 4*np.matrix(np.ones((self.d, self.d), float))
...
d = 2
mn = np.ones((d, 1), float)
mn = 3*mn
var = diag(np.ones((d, 1), float)) + 4*np.matrix(np.ones((d, d), float))
print(mn, var)
cset = box(d, [(0, 2)]*d)
sset = space(d)
mlt = multivariateGaussian(d, mn, var, cset)
print(mlt.getSample())
l = []
for i in range(1000):
...
trunc = truncatedLearning(d, cset, mlt, epsilon=0.1)
